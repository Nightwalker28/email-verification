...
class ApiKey(db.Model):
    __tablename__ = 'api_keys'  # Define the table name

    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('users.user_id'), nullable=False)
    api_key = db.Column(db.Text, unique=True, nullable=False)
    name = db.Column(db.String(128), nullable=False)  # New field for the name of the API key

    user = db.relationship('User', backref=db.backref('api_keys'))

    def __repr__(self):
        return f'<ApiKey {self.api_key}, Name: {self.name}>'
