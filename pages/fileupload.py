...
def process_emails(emails,user=None, force=False):
    """Processes the list of emails by performing verification."""
    results = []
    for email in emails:
        logger.info(f'Processing email: {email}')
        try:
            verification_details = perform_email_verification(email, providers, roles, user=user, force_live_check=force, increment_count=True)
