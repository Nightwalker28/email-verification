...
from pages.models import create_temp_user, get_or_create_searched_email,add_verified_email_for_user,get_last_checked_emails,db,User,UserUpload,Summary,TempUser,create_user,ApiKey
...
from pages.apis import api_verify,create_api_key,edit_api_key,delete_api_key,require_api_key,api_forceverify,api_upload
...
def download_file(unique_filename):
    # Query the database for the uploaded file entry
    upload_entry = UserUpload.query.filter_by(unique_filename=unique_filename).first()
    if not upload_entry:
        logger.error(f'No record found for file: {unique_filename}')
    ...
    # Use Content-Disposition header to support direct downloads with the original filename
    try:
        return send_from_directory(
            UPLOAD_FOLDER,
            unique_filename,
            as_attachment=True,
            download_name=upload_entry.original_filename  # Correct filename
        )
    except Exception as e:
        logger.error(f'Error serving file {unique_filename}: {str(e)}')
...
@app.route('/apis')
def api_keys():
    user_id = session.get('user')  # Assuming you store the user ID in the session
    if not user_id:
        return redirect(url_for('indexview'))  # Redirect to login if not authenticated
...
@app.route('/edit_api', methods=['POST'])
def edit_api():
    api_key_id = request.form['id']
    new_name = request.form['name']
    user_id = session.get('user')  # Assuming you have the user ID in the session 
...
@api_blueprint.route('/forceverify', methods=['POST'])
@require_api_key
def apiforceverify():
