...
from pages.users import reset_verification_attempts,get_verification_attemps
...
def homeview():
    if 'user' in session:
        ...
        return render_template('home.html', last_checked_emails=res, attempts=attempts, show_recent_result=show_recent_result)   
    return redirect('/')
...
def google_auth():
    try:
        ...
        jsonify("An authentication error occurred.")
        return redirect('/')
